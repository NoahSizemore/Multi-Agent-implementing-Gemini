...
# ---------------------------------------------------------------------------
# Module 4: Code structure analysis
# ---------------------------------------------------------------------------
async def analyze_code_structure(code: str, tool_context: ToolContext) -> Dict[str, Any]:
    ...
    try:
        ...
        # ast.parse is CPU-bound and can raise SyntaxError, which is
        # propagated by the await and handled below.
        ...
        # Store code and analysis so other agents in the pipeline can access them
...
def _extract_code_structure(tree: ast.AST, code: str) -> Dict[str, Any]:
    ...
    Runs in a thread pool for CPU-bound work.
    """
    functions = []
    classes = []
    imports = []
...
async def check_code_style(code: str, tool_context: ToolContext) -> Dict[str, Any]:
    ...
    try:
        # Retrieve code from state if not provided (pipeline use passes "")
        if not code:
        ...
        # Run the synchronous style check in a thread pool
        loop = asyncio.get_event_loop()
        with ThreadPoolExecutor() as executor:
        ...
        # Store results in state for later agents
...
# ---------------------------------------------------------------------------
# Module 5: Feedback synthesizer tools
# ---------------------------------------------------------------------------
